      ...
      className={clsx(
        "absolute h-[30px] w-[40px] border flex items-center justify-center text-xs",
        {
          ...
          "bg-purple-50 text-purple-700 dark:bg-purple-950 dark:text-purple-300":
            status === LOCK,
          "bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300": status === PAPA,
          "bg-amber-50 text-amber-700 dark:bg-amber-950 dark:text-amber-300": status === RSVD,
        },
      )}
      id={"s-" + nr}
    >
      ...
      {view === "view0" && status !== LOCK && status !== 0 && <Car size={24} />}
      {view === "view1" && status}
      {view === "view2" && nr}
      {view === "view3" && size}
    </div>
